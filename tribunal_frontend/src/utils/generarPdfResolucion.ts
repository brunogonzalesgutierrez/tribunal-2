...
export async function generarPdfResolucion(params: {
  ...
  const { expediente: exp, resolucion: r, partes, vocales, denuncia } = params;
  ...
  const dibujarPiePagina = () => {
    ...
    doc.text(`Generado el ${new Date().toLocaleString("es-BO")}`, ML, piePaginaY + 5);
  };
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(...GRIS);
  doc.text(`${r.idTipoRes?.nombre ?? "Resolución"} — ${fmtFecha(r.fechaResolucion)}`, ML + 8, y + 14);
  doc.text(
  ...
  fila2col("N° de Expediente", exp?.numeroExpediente ?? "—", "Año", String(exp?.ano ?? "—"));
  fila2col("Tipo de Proceso", exp?.idTipoProceso?.nombre ?? "—", "Estado del Expediente", exp?.idEstadoExpediente?.nombreEstado ?? "—");
  fila2col("Sala", exp?.idSala?.nombreSala ?? "—", "Instancia", exp?.idSala?.idTribunal?.instancia ?? "—");
  fila2col("Fecha de Ingreso", fmtFechaCorta(exp?.fechaIngreso), "Fecha de Conclusión", fmtFechaCorta(exp?.fechaConclusion));
  ...
  // ── Antecedentes (solo si viene de denuncia) ──────────────────────────
  if (denuncia) {
    checkY(20);
    seccion("III. Antecedentes del Proceso Disciplinario");

    fila2col(
      "N° Denuncia", denuncia.numeroDenuncia ?? "—",
      "Fecha de denuncia", fmtFechaCorta(denuncia.fechaDenuncia)
    );
    fila2col(
      "Denunciante",
      `${denuncia.denunciante?.nombre ?? ""} ${denuncia.denunciante?.primerApellido ?? ""}`.trim() || "—",
      "C.I.", denuncia.denunciante?.numeroDocumento ?? "—"
    );
    fila2col(
      "Denunciado",
      `${denuncia.denunciado?.nombre ?? ""} ${denuncia.denunciado?.primerApellido ?? ""}`.trim() || "—",
      "Estamento", denuncia.tipoDenunciado ?? "—"
    );
    fila2col(
      "Fecha del hecho", fmtFechaCorta(denuncia.fechaHecho),
      "Tipo de resolución", denuncia.tipoResolucion === "SANCIONATORIA" ? "Sancionatoria" : "Absolutoria"
    );

    if (denuncia.tipoSancion) {
      ...
      doc.setFontSize(7);
      doc.setTextColor(...GRIS);
      doc.text("SANCIÓN IMPUESTA", ML, y);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(8.5);
      doc.setTextColor(...OSCURO);
      doc.text(
        `${SANCION_LABEL[denuncia.tipoSancion] ?? denuncia.tipoSancion}${denuncia.detalleSancion ? " — " + denuncia.detalleSancion : ""}`,
        ML, y + 4.5
      );
      y += 10;
    }
    ...
    doc.setFontSize(7);
    doc.setTextColor(...GRIS);
    doc.text("DESCRIPCIÓN DE LOS HECHOS DENUNCIADOS", ML, y);
    y += 5;
    const hechosLines = doc.splitTextToSize(denuncia.descripcion ?? "—", CUERPO_W - 4);
    checkY(hechosLines.length * 5.2 + 6);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8.5);
    doc.setTextColor(...OSCURO);
    doc.text(hechosLines, ML, y, { lineHeightFactor: 1.4 });
    y += hechosLines.length * 5.2 + 8;
  }

  // ── Parte Dispositiva ─────────────────────────────────────────────────
  checkY(20);
  const numDisp = denuncia
    ? (vocales.length > 0 ? "V" : partes.length > 0 ? "IV" : "III")
    : (vocales.length > 0 ? "IV" : partes.length > 0 ? "III" : "II");
  ...
  if (r.fundamentacion) {
    checkY(20);
    const numFunda = ({ "V": "VI", "IV": "V", "III": "IV", "II": "III" } as Record<string,string>)[numDisp] ?? "III";
  ...
  checkY(20);
  seccion("VI. Información Adicional de la Resolución");
  fila2col("Tipo de Resolución", r.idTipoRes?.nombre ?? "—", "Código", r.idTipoRes?.codigo ?? "—");
  fila2col(
  ...
  const firmaW = CUERPO_W / (vocales.length > 0 ? Math.min(vocales.length, 3) : 1);
  ...
  for (let i = 1; i <= totalPaginas; i++) {
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(...AZUL);
    doc.text(`Página ${i} de ${totalPaginas}`, PW - MR, piePaginaY + 5, { align: "right" });
  }
