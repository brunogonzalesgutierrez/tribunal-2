...
import {
  GET_TIPOS_PROCESO,
  CREAR_TIPO_PROCESO,
  ACTUALIZAR_TIPO_PROCESO,
  ELIMINAR_TIPO_PROCESO,
...
export default function TiposProcesoPage() {
  ...
  const [actualizarTipo] = useMutation(ACTUALIZAR_TIPO_PROCESO);
  const [eliminarTipo] = useMutation(ELIMINAR_TIPO_PROCESO);
  const { executeCreate, executeUpdate, executeDelete, toast } = useCrudNotifications("Tipo de proceso");
  ...
  const abrirEditar = (t: TipoProceso) => {
    setEditando(t);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Catálogo de tipos de proceso judicial • {tipos.length} registros
          </p>
        </div>
        <button
          onClick={abrirCrear}
        ...
        <div className="bg-white dark:bg-slate-800/90 rounded-2xl border border-gray-200 dark:border-slate-700 p-5 shadow-lg hover:shadow-xl transition-all group">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-sky-600 dark:text-sky-400 mt-2">{tipos.length}</p>
            </div>
          ...
          <div className="mt-4 pt-3 border-t border-gray-100 dark:border-slate-700">
            <p className="text-xs text-gray-500 dark:text-gray-400">Tipos de proceso registrados</p>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-800/90 rounded-2xl border border-gray-200 dark:border-slate-700 p-5 shadow-lg hover:shadow-xl transition-all group">
          <div className="flex items-center justify-between">
                ...
                paginados.map(t => (
                  <tr key={t.idTipoProceso} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors group">
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-sky-100 dark:bg-sky-900/30 text-sky-700 dark:text-sky-400 font-mono">
